...
export class CloudSyncService {
     ...
     */
    async fetchFromCloud(limit = 100) {
        if (!this.syncEnabled || !this.r2Endpoint) {
            return [];
        }
        try {
            const response = await fetch(`${this.r2Endpoint}/list?prefix=schools/${this.schoolId}/users/${this.userId}/results/&limit=${limit}`, {
                headers: {
                    'Authorization': `Bearer ${await this.getAuthToken()}`
                }
            });
...
export function getCloudSyncService(schoolId, userId) {
