...
type DocType = "id-card" | "report-card" | "pass-out";

export const DocumentHub = () => {
    const { schoolId, displayName } = useBrandingStore();
    const [selectedType, setSelectedType] = useState<DocType>("id-card");
    ...
    return (
        <div className="p-6 max-w-5xl mx-auto space-y-8">
            <header className="space-y-2">
                ...
                <p className="text-slate-400">Generate high-quality, 300 DPI printable documents for your school instantly.</p>
            </header>

            {/* Selector */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <DocTypeCard
                    active={selectedType === "id-card"}
                    onClick={() => setSelectedType("id-card")}
                    icon={CreditCard}
                    title="ID Cards"
                    description="Professional photo IDs for students and staff."
                />
                <DocTypeCard
                    active={selectedType === "report-card"}
                    onClick={() => setSelectedType("report-card")}
                    icon={FileText}
                    title="Report Cards"
                    description="Printable academic reports with student photos."
                />
                <DocTypeCard
                    active={selectedType === "pass-out"}
                    onClick={() => setSelectedType("pass-out")}
                    icon={LogOut}
                    title="Pass-out Slips"
                    description="Security-verified slips for students leaving school."
                />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 space-y-6">
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-slate-400 mb-1">Student/Staff Name</label>
                            <input
                                type="text"
                                value={studentName}
                                onChange={(e) => setStudentName(e.target.value)}
                                placeholder="Full Name"
                                className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-2 text-white focus:ring-2 focus:ring-blue-500 transition-all"
                            />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-400 mb-1">Registration ID</label>
                            <input
                                type="text"
                                value={studentId}
                        ...
                        {selectedType === "pass-out" && (
                            ...
                            >
                                <div>
                                    <label className="block text-sm font-medium text-slate-400 mb-1">Reason for Leaving</label>
                                    <input
                                        type="text"
                                        value={reason}
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-400 mb-1">Departure Time</label>
                                    <input
                                        type="text"
                                        value={departureTime}
                    ...
                    <AnimatePresence>
                        {status && (
                            <motion.div
                                ...
                                exit={{ opacity: 0 }}
                                className={`p-4 rounded-xl flex items-center gap-3 ${status.type === 'success' ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'bg-red-500/10 text-red-500 border border-red-500/20'
                                    }`}
                            >
                ...
                <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 flex flex-col items-center justify-center text-center space-y-4">
                    ...
                    <div>
                        <h3 className="text-lg font-semibold">Print-Ready Quality</h3>
                        <p className="text-sm text-slate-500 max-w-xs mx-auto">
                            All documents are generated at 300 DPI. Perfect for laser or inkjet printing on standard paper or PVC card printers.
                        </p>
                    </div>
                </div>
            </div>
...
const DocTypeCard = ({ active, onClick, icon: Icon, title, description }: any) => (
    <button
        onClick={onClick}
        className={`p-5 rounded-2xl border-2 text-left transition-all ${active
                ? "bg-blue-600/10 border-blue-600 shadow-lg shadow-blue-900/20"
                : "bg-slate-900/40 border-slate-800 hover:border-slate-700"
            }`}
    >
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 ${active ? "bg-blue-600 text-white" : "bg-slate-800 text-slate-400"
            }`}>
            <Icon size={20} />
        </div>
        <h3 className={`font-bold ${active ? "text-white" : "text-slate-200"}`}>{title}</h3>
