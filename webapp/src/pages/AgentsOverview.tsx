const agents = [
  {
    ...
    mission:
      "Delivers strategic briefings, growth playbooks, and executive decisions using Clarity’s intelligence.",
  },
  {
    name: "Command Intelligence Agent – Clarity Engine",
    mission:
      "Turns natural language commands into precise workflows, coordinating all other agents.",
  },
  {
    name: "Document Intelligence Agent – RAG Master",
    mission:
      "Digitizes handwritten forms, routes data to the right systems, and builds searchable knowledge bases.",
  },
  {
  ...
  },
  {
    ...
    mission:
      "Runs fee collection OODA loops, mobile money reconciliation, and compliance reporting.",
  },
  {
    name: "Academic Operations Agent – Educational Excellence Manager",
    mission:
      "Generates learning insights, predicts at-risk students, and optimizes curriculum delivery.",
  },
  {
...
export const AgentsOverview = () => (
  <section className="space-y-6">
    <header className="space-y-2">
      <h1 className="text-2xl font-semibold">Meet the Angels AI Crew</h1>
      <p className="text-slate-300">
        Nine specialized agents working together with the Clarity engine to deliver a full-stack
        school operations Ferrari.
      </p>
    </header>
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      {agents.map((agent) => (
        <article
          key={agent.name}
          className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6 space-y-2"
        >
          <h2 className="text-lg font-semibold">{agent.name}</h2>
          <p className="text-sm text-slate-300">{agent.mission}</p>
        </article>
      ))}
    </div>
  </section>
