...
class MultiSchoolService:
    ...
    def link_school(self, school_code: str, role: str) -> Dict[str, Any]:
        """
        Link user to a new school using a school code
        
        Args:
            school_code: 8-character unique school code
            role: User's role at this school
        
        Returns:
            Success status and approval status
        """
        # 1. Verify school exists by code
        school_query = "SELECT id, name FROM schools WHERE school_code = %s"
        school = self.db.execute_query(school_query, (school_code,), fetch=True)
        ...
        # Students and Parents are auto-approved for frictionless entry
        # Teachers and Staff require admin approval
        status = 'active'
        if role in ['teacher', 'staff', 'admin']:
            status = 'pending'
        
        # 3. Create/Update role assignment
        try:
            self.db.execute_query(
                """
                ...
                """,
                (self.user_id, school_id, role, status)
            )
            
            message = "Welcome! You have been successfully linked to the school."
            if status == 'pending':
                message = "Your request to join as staff has been sent to the school admin for approval."
            ...
            return {
                "success": False,
                "error": f"Failed to link school: {str(e)}"
            }
